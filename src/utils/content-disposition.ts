...
export function contentDisposition(
  ...
  const opts = options || { fallback: false };

  // get type
  const type = opts.type || 'attachment';

  // get parameters
  const fileName = createFileName(filename, opts.fallback);

  // format into string
  return format(new ContentDisposition(type, fileName));
}
 ...
 * @private
 */
function createFileName(filename: string, fallback: string | boolean): string {
  ...
  if (isQuotedString || hasFallback) {
    return hasFallback ? fallbackName : name;
  }
}
...
export function parse(string) {
  ...
  return new ContentDisposition(type, params.filename);
}
